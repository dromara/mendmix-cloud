 ...
 */
public class RwRouteHandler implements PluginInterceptorHandler {
	...
	public Object onInterceptor(OnceContextVal invocation) throws Throwable {
		...
		if(ms.getSqlCommandType().equals(SqlCommandType.SELECT)){
			...
			if(ms.getId().contains(SelectKeyGenerator.SELECT_KEY_SUFFIX)){				
				...
				logger.debug("<framework-logging> Method[{} use Slave Strategy..",ms.getId());
			}
		}else{
			logger.debug("<framework-logging> Method[{}] use Master Strategy..",ms.getId());
			MybatisRuntimeContext.useMaster();
		}
