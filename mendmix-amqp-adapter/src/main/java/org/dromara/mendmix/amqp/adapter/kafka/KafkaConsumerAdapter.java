...
public class KafkaConsumerAdapter extends AbstractConsumer {
	...
	public KafkaConsumerAdapter(MQContext context,Map<String, MessageHandler> messageHandlers) {
		...
		timeoutDuration = Duration.ofMillis(ResourceUtils.getLong(context.getInstance() + ".amqp.fetch.timeout.ms",100));
	}
	...
	public void start() throws Exception {
		logger.info("<startup-logging>  KafkaConsumer start Begin..");
			...
			ConsumerRebalanceListener listener = new ConsumerRebalanceListener() {
				...
				public void onPartitionsAssigned(Collection<TopicPartition> partitions) {
					...
					for (TopicPartition partition : partitions) {
						uncommitOffsetStats.put(partition, new OffsetAndMetadataStat(0,0));
					}
				}
		...
		logger.info("<startup-logging>  KafkaConsumer start End -> subscribeTopics:{}",topicNames);
		 ...
		 while(iterator.hasNext()) {
			 ...
			 if(item.headers() != null) {
				 item.headers().forEach(h -> {
					 message.addHeader(h.key(), new String(h.value()));
				 });
			 }
		...
		try {						
			...
			if(assignment.contains(topicPartition)){
				if(resetOffset > 0 && resetOffset < metadata.offset()){								
			        ...
			        logger.info("<framework-logging> seek topicPartition[{}] from {} to {}",topicPartition,metadata.offset(),resetOffset);
				}
			}
		} catch (Exception e) {
			logger.warn("<framework-logging> try seek topicPartition["+topicPartition+"] offsets error");
		}
	...
	private Properties buildConfigs() {
        ...
        if (!result.containsKey(ConsumerConfig.CLIENT_ID_CONFIG)) {
            result.put(ConsumerConfig.CLIENT_ID_CONFIG, context.getGroupName() + GlobalContext.getWorkerId());
        }
        
        //每批次最大拉取记录
        if (!result.containsKey(ConsumerConfig.MAX_POLL_RECORDS_CONFIG)) {
        	int maxPollRecords = context.getMaxProcessThreads() > 1 ? context.getMaxProcessThreads()/2 : 1;
            result.put(ConsumerConfig.MAX_POLL_RECORDS_CONFIG, maxPollRecords);
        }
