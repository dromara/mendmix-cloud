...
public class OauthEndpointController implements InitializingBean {
	...
	public void afterPropertiesSet() throws Exception {
		Set<String> types = ResourceUtils.getMappingValues("mendmix.security.oauth").keySet();
		
		OauthClient client;
		for (String type : types) {
			client = new OauthClient();
			client.setType(type);
			client.setClientId(ResourceUtils.getAndValidateProperty(String.format("mendmix.security.oauth[%s].clientId", type)));
			client.setClientSecret(ResourceUtils.getAndValidateProperty(String.format("mendmix.security.oauth[%s].clientSecret", type)));
			client.setAuthorizeUrl(ResourceUtils.getAndValidateProperty(String.format("mendmix.security.oauth[%s].authorizeUrl", type)));
			client.setAccessTokenUrl(ResourceUtils.getAndValidateProperty(String.format("mendmix.security.oauth[%s].accessTokenUrl", type)));
			client.setUserInfoUrl(ResourceUtils.getAndValidateProperty(String.format("mendmix.security.oauth[%s].userInfoUrl", type)));
